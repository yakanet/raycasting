...
function renderCeilingIntoImageData(imageData, player, scene) {
    ...
    for (let y = SCREEN_HEIGHT / 2; y < SCREEN_HEIGHT; ++y) {
        ...
        for (let x = 0; x < SCREEN_WIDTH; ++x) {
            ...
            imageData.data[(sz * SCREEN_WIDTH + x) * 4 + 0] = tile.data[0] / brightness;
            imageData.data[(sz * SCREEN_WIDTH + x) * 4 + 1] = tile.data[1] / brightness;
            imageData.data[(sz * SCREEN_WIDTH + x) * 4 + 2] = tile.data[2] / brightness;
            imageData.data[(sz * SCREEN_WIDTH + x) * 4 + 3] = tile.data[3];
        }
    }
}
    ...
    for (let y = SCREEN_HEIGHT / 2; y < SCREEN_HEIGHT; ++y) {
        ...
        for (let x = 0; x < SCREEN_WIDTH; ++x) {
            ...
            if (!tile)
                continue;
            const brightness = Math.sqrt(player.position.sqrDistanceTo(t));
            imageData.data[(y * SCREEN_WIDTH + x) * 4 + 0] = tile.data[0] / brightness;
            imageData.data[(y * SCREEN_WIDTH + x) * 4 + 1] = tile.data[1] / brightness;
            imageData.data[(y * SCREEN_WIDTH + x) * 4 + 2] = tile.data[2] / brightness;
            imageData.data[(y * SCREEN_WIDTH + x) * 4 + 3] = tile.data[3];
        }
    }
}
